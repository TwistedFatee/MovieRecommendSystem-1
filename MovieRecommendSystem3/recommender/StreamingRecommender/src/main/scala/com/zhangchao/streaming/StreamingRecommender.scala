package com.zhangchao.streaming
...
object ConnHelper extends Serializable{
  ...
  //lazy val mongoClient=MongoClient("mongodb://hadoop102:27017/recommender")
...
object StreamingRecommender {
  ...
  val sc=spark.sparkContext
   ...
   val simMovieMatrixBroadCast = sc.broadcast(simMovieMatrix)
    ...
    //将原始数据UID|MID|SCORE|TIMESTAMP转换为评分流
      ...
      rdds=>rdds.foreach{
        case (uid,mid,score,timestamp)=>{
         ...
         println("rating data coming!!>>>>>>>>>>>>>>>>>>>>")
          //1. 从redis里获取用户最近的K次评分，保存成Array[(mid,score)]
          ...
          //3. 对每个备选电影，计算推荐优先级，得到当前用户的实施推荐列表，Array[(mid,score)]
  ...
  //从redis里获取用户最近的K次评分，保存成Array[(mid,score)]
  ...
  //3. 对每个备选电影，计算推荐优先级，得到当前用户的实施推荐列表，Array[(mid,score)]
  def computeMovieScores(candidateMovies: Array[Int], userRecentlyRatings: Array[(Int, Double)], simMoves: collection.Map[Int, Map[Int, Double]]) = {
    //定义一个arraybuffer。用于保存每一个备选电影的基础得分
    val scores: ArrayBuffer[(Int, Double)] = ArrayBuffer[(Int, Double)]()

    //定义一个hashmap，用于保存每一个备选电影的增强减弱因子
    ...
    for (candidateMovie <- candidateMovies; userRecentlyRating <- userRecentlyRatings ) {
      //拿到备选电影和最近评分电影的相似度
       val simScore: Double = getMoviesSimScore(candidateMovie, userRecentlyRating._1, simMoves)
  ...
  //获取两个电影之间的相似度
  def getMoviesSimScore(mid1: Int, mid2: Int, simMoves: collection.Map[Int, Map[Int, Double]]): Double ={
    simMoves.get(mid1)match {
      //TODO 为什么是some
